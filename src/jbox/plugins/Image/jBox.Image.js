...
jQuery(document).ready(function () {
  ...
  new jBox.plugin('Image', {
    ...
    imageLabel: 'jBox-image-label',        // The attribute where jBox gets the image label from, e.g. title="My label"
    imageFade: 360,             // The fade duration for images in ms
    imageSize: 'contain',       // How to display the images. Use CSS background-position values, e.g. 'cover', 'contain', 'auto', 'initial', '50% 50%'
    imageCounter: true,        // Set to true to add an image counter, e.g. 4/20
    ...
    _onInit: function ()
    {
      ...
      var appendImage = function (gallery, id, preload, open, error)
      {
        ...
        }).attr("src", this.images[gallery][id].src).attr("target","_blank").appendTo(this.content);

        // Create labels
        jQuery('<div/>', {
          id: 'jBox-image-label-' + gallery + '-' + id,
          'class': 'jBox-image-label' + (open ? ' active' : '')
        }).html(this.images[gallery][id].label).appendTo(this.imageLabel); //.hover(function () { $(this).toggleClass('expanded'); })
